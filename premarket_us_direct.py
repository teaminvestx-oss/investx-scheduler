...
import os, io, math, json, traceback
...
# Pares: (nombre, futuro, cash, proxy cash si hiciera falta)
PAIRS = [
...
def yahoo_last_price(sym: str, rng="1d", intv="1m"):
    """Último close no-nulo de velas (futuros: ‘1d/1m’, índices cash: ‘5d/1d’)."""
...
def prev_cash_close(sym: str, proxy: str|None):
    # cierra del día previo del cash (o ETF proxy)
    for s in (sym, proxy) if proxy else (sym,):
        try:
...
def draw_image(rows, ts_local):
    ...
    for i,r in enumerate(rows):
        ...
        f=r["fut"]; c=r["cash"]
        d.text((x+30,y+170), f"Futuro: {f:.2f}" if isinstance(f,(int,float)) else "Futuro: —", font=font(22), fill=MUT)
        d.text((x+30,y+200), f"Cierre cash: {c:.2f}" if isinstance(c,(int,float)) else "Cierre cash: —", font=font(22), fill=MUT)
    return im
...
def interpretation(rows):
    ...
    out=[f"{tone} en futuros: media {mean:+.2f}%.",
...
def main():
    ...
    # Ventana 11:55–12:15 (o envío forzado/manual)
    ...
    cap = ["<b>Premarket USA</b>"] + [f"• <b>{r['name']}</b>: {'—' if r['pct'] is None else f'{r['pct']:+.2f}%'}" for r in rows]
    cap.append(""); cap.append(interpretation(rows))
    send_photo(img, "\n".join(cap))
    print("[ok] enviado.")
