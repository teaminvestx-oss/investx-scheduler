...
// ✅ Versión robusta: nunca falla, aunque Node esté en UTC
function isMonday() {
  const now = new Date();
  const nowTz = new Date(now.toLocaleString('en-US', { timeZone: TZ }));
  const dow = nowTz.getDay(); // 0=domingo, 1=lunes...
  return dow === 1;
}
...
async function fetchWithTimeout(url, { timeoutMs = 20000, retries = 1, headers = {}, method = 'GET', body } = {}) {
  ...
  for (let i = 0; i <= retries; i++) {
    ...
    try {
      ...
      clearTimeout(t);
      last = e;
      if (VERBOSE) console.log('fetch fail', url, e.message);
...
async function getFFJson(path) {
  ...
  for (const base of FF_HOSTS) {
    ...
    try {
      ...
      lastErr = e;
      if (VERBOSE) console.log('FF host fail:', base, e.message);
    }
  }
