...
# - Top 10 Subidas/Caídas ordenadas por % desde Finviz
# - Heatmap jerárquico estilo Finviz: Sector → Industria → Tickers
# - Usa Playwright (Chromium headless) para sortear Cloudflare; fallback a requests
# - Envía foto por Telegram (fallback a sendDocument); si no hay datos, envía solo texto y sale OK
...
LIMIT_PER_INDEX = 60            # filas por índice (más variedad sector/industria)
TOP_N = 10
...
def get_html(url: str) -> str:
    """Carga la página como navegador real (Playwright) y, si falla, usa requests con backoff."""
    # 1) Intento con Playwright (Chromium headless)
    if USE_PLAYWRIGHT:
        try:
    ...
    # 2) Fallback: requests + cookies y reintentos
    try:
    ...
    print(f"[WARN] No se pudo obtener HTML de {url}: {last}")
    return ""

# ---------------- Parseo de Finviz ----------------
def pick_finviz_table(html_text: str) -> pd.DataFrame:
    """Tabla real del screener (descarta tablas de filtros/banners)."""
...
def finviz_fetch(index_code: str, top=True, limit=LIMIT_PER_INDEX) -> pd.DataFrame:
    """Lee Finviz empezando en página 1 y ordenando por % explícitamente."""
    s = "ta_topgainers" if top else "ta_toplosers"
    order = "o=-change" if top else "o=change"  # ganadores desc, perdedores asc
...
for name, code in INDICES.items():
    try:
        ...
        # Pequeña espera para parecer “humano”
        time.sleep(1.2)
    except Exception as e:
        print(f"[WARN] {name}: {e}")
...
# Si no hay datos válidos, envía solo texto y termina OK
if movers.empty:
...
# Paleta “Finviz-like”: rojo↔verde centrada en 0
...
for i, sec in enumerate(sectors):
    ...
    # INDUSTRIAS (size = suma de mcap; si no hay mcap, proxy = nº de tickers)
    ind_group = sub_sec.groupby("Industry", as_index=False).agg(dummy=("Ticker", "count"))
    ...
    if (ind_group["mcap_sum"]<=0).all():
        ind_group["mcap_sum"] = ind_group["dummy"].astype(float)
    ...
    for rect, ind in zip(rects_ind, ind_group["Industry"]):
        ...
        # TICKERS dentro de la industria (size = mcap; si no hay, uniformes)
...
# ---------------- Export a JPEG y envío (con fallback) ----------------
buf_png = io.BytesIO()
